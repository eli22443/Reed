...
if __name__ == "__main__":
    ...
    if len(candidates) > 0:
        ...
        for i, candidate in enumerate(candidates):
            ...
            # בדיקה אם זה ההודעה המקורית
            if len(candidate) == len(original_msg) and all(
                candidate[i] == original_msg[i] for i in range(len(original_msg))
            ):
                print(f"  [OK] Candidate {i+1} matches the original message!")

        # בדיקה אם ההודעה המקורית ברשימה
        original_found = any(
            len(c) == len(original_msg)
            and all(c[i] == original_msg[i] for i in range(len(original_msg)))
            for c in candidates
        ...
        if original_found:
            print("\n[SUCCESS] Original message successfully recovered!")
        else:
            print(
                "\n⚠ Original message not found in candidates (may need more iterations or different parameters)"
            )
    else:
        print("\n⚠ No candidates found. The algorithm may need adjustment.")
    ...
    # נבדוק עם 4 שגיאות (בגבול היכולת של פענוח ייחודי)
    # זה אמור לעבוד ב-100% מהמקרים
    print("\nTest: 4 errors (at unique decoding limit)")
    print("This test verifies that list decoding finds the original message.")
    try:
        test_passed = test_list_decoding_finds_original(
            num_errors=4, num_trials=10, assert_success=True
        )
        print(
            "[SUCCESS] Test passed! List decoding successfully finds original message."
        )
